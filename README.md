...
- 3.2초 정지 후 형태 비교. 마지막에 움직인 별을 기준으로 연결선이 한 방향으로 이어지며 서서히 나타나고, 뒤이어 형태에 대응하는 삽화가 천천히 밝아집니다. 삽화는 항상 연결된 별 모양보다 크게 유지되며, 별 배치가 아주 넓어지면 카메라가 뒤로 물러나는 듯한 연출로 삽화 크기 자체는 일정하게 유지합니다. 별을 다시 움직이면 이전 연결선은 서서히 사라집니다. 수동 ‘형태 발견하기’와 자동 인식 끄기도 지원합니다.
...
- 앰비언트 사운드는 여름 저녁 귀뚜라미 소리(합성음)이며, 움직임 강도(다인원 감지·직접 편집)에 반응해 은은한 배음이 밝아지고, 정각마다 초신성처럼 화면 곳곳이 잠깐 폭발했다 가라앉는 연출이 있습니다(‘고요한 화면’ 설정 시 꺼짐). 자세가 실루엣으로 드러나는 순간에는 셔터음도 함께 납니다.
