...
export function matchShapes(points, shapes) {
  ...
  return shapes
    .map((shape) => {
      const target = normalize(shape.points);
      let best = { cost: Infinity, angle: 0, mirror: 1 };
      ...
      for (const mirror of [1, -1]) {
        for (let i = 0; i < 32; i++) {
          ...
          if (value < best.cost) best = { cost: value, angle, mirror };
        }
      }
      for (let step = Math.PI / 32; step > 0.0005; step /= 2) {
        for (const angle of [best.angle - step, best.angle + step]) {
          const value = cost(angle, best.mirror);
          if (value < best.cost) best = { ...best, cost: value, angle };
        }
      }
...
export function buildConnections(points, match) {
  ...
  const bridges = points
    ...
    .sort((a, b) => a.d - b.d);
  for (const { a, b } of bridges) if (root(a) !== root(b)) add(a, b);
  return edges;
}
